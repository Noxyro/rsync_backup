...
function run_remote_shell_command() {
    case $# in
    2)
      ssh -i "/home/noxyro/.ssh/.id_ed25519" "${2}" "${1}";;
    3)
      ssh -i "/home/noxyro/.ssh/.id_ed25519" -l "${3}" "${2}" "${1}";;
    4)
      ssh -i "/home/noxyro/.ssh/.id_ed25519" -p "${4}" -l "${3}" "${2}" "${1}";;
    *)
      return 1
  esac
}
...
function find_remote_directories_by_name_with_time_prefix() {
  case $# in
    3)
      ssh -i "/home/noxyro/.ssh/.id_ed25519" "${3}" "find ${1} -maxdepth 1 -type d -name ${2} -printf \"%T@ %p\n\"";;
    4)
      ssh -i "/home/noxyro/.ssh/.id_ed25519" -l "${4}" "${3}" "find ${1} -maxdepth 1 -type d -name ${2} -printf \"%T@ %p\n\"";;
    5)
      ssh -i "/home/noxyro/.ssh/.id_ed25519" -p "${5}" -l "${4}" "${3}" "find ${1} -maxdepth 1 -type d -name ${2} -printf \"%T@ %p\n\"";;
    *)
      echo "-1"
  esac
}
